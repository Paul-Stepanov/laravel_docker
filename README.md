...
Это Laravel-приложение, развернутое с использованием Docker. Проект включает в себя контейнеры для Nginx, PHP, PostgreSQL и
PhpMyAdmin.
...
- `DB_PASSWORD` - пароль для доступа к PostgreSQL
...
- `DOMAIN_PROD` - переменная для конфига nginx, определяет откуда будут тянуться изображения для тестовой площадки (
  proxy_pass)
- `USE_HTTPS` - если вы работаете локально, то должна содержать `false`
- `HTTPS_SCHEMA` - если вы работаете локально, то должна содержать `false`
...
4. После запуска контейнеры будут доступны по следующим адресам:
    ...
    - PostgreSQL: порт 5432 (доступен локально)
...
- PHP 7.2 FPM с необходимыми расширениями
...
4. Контейнер `db` запускает PostgreSQL сервер

## Работа с базой данных

База данных автоматически инициализируется с помощью скриптов из директории `docker/postgres/initdb/`. Положите файлы с
SQL-скриптами(бекап БД) в директорию `docker/postgres/initdb/` и они будут выполнены при инициализации базы данных.
...
- Все данные PostgreSQL сохраняются в Docker volume и сохраняются между перезапусками
